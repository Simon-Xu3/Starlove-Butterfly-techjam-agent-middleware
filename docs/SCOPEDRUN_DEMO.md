...
| --- | --- | --- |
| [Recorded real ModelArk live allow path](evidence/p3-resource-capsule-live-e2e-test-2026-08-30.md) | At its named revision: production Web/API to Docker-backed container Runtime, explicit `orders-incident` Delegation, a real ModelArk response with fixture-only facts, correlated ALLOW Receipt with generation `1` and `runnerStarted: yes`, and unchanged post-Run fixture hashes. | Subsequent revisions, every negative control, or production-grade isolation and authentication. |
| [Issue #33 security/lifecycle audit](evidence/issue-33-final-audit-2026-08-31.md) | Issue #33 security/lifecycle fixes, deterministic suites, real-container Agent-state and Resource boundary, browser UI, dependency, Compose, and documentation status at that audit revision. | Production-grade isolation or authentication, Terraform deployment without local Terraform, or a successful live Ark answer from that audit itself. |
